        ...
        if i.getAttribute("type") == "UML - Class":
                    ...
                    if j.getAttribute("name") == "attributes":
                        for l in j.getElementsByTagName("dia:composite"):
                            if l.getAttribute("type") == "umlattribute":
                                ...
                                varch = v2c.search(tc)
                                if tc.replace(" ", "").startswith("ManyToManyField(") or tc.replace(" ", "").startswith("ForeignKey(") or tc.replace(" ", "").startswith("OneToOneField("):
            ...
            for j in a:
                if len(j.childNodes[0].data[1:-1]):
                    imports += "from %s.models import *" % j.childNodes[0].data[1:-1]
