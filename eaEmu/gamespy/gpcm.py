...
class Comrade(LoginServer):
    ...
    def recv_pinvite(self, msg):
        ...
        try:
            ...
            self.factory.conns[persona.user].sendMsg(
                MessageFactory.getMessage(
                    [
                        ("bm", 101),
                        ("f", persona.id),  ## f = from?
        ...
        except Persona.DoesNotExist as ex:  ## TODO, FIXME: sometimes (always firsttime for coop invite)I get weird ids, including the BE int of my old publicip?? where are these vals coming from
