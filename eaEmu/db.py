...
class _StatsAspect:
    ...
    def dumpFields(self, fields, withNames=False):
        ...
        return ":\\" + "\\".join(sum([[k, str(fieldVals[k])] for k in fields] if withNames else [[str(fieldVals[k])] for k in fields], []))
...
def syncAccount(username):
    ...
    return phpbb_db.runQuery('SELECT user_id, username, user_password, user_email FROM phpbb_users WHERE username_clean LIKE LOWER("{0}") or user_email LIKE LOWER("{0}")'.format(username)).addCallbacks(cbRunQuery, ebRunQuery)
